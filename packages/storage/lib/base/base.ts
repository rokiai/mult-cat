...
export const createStorage = <D = string>(
  ...
  get()
    ...
    .catch(error => {
      if (!isExtensionContextInvalidated(error)) {
        console.warn('[storage] initial get failed', key, error);
      }
      cache = fallback;
      initialCache = true;
      _emitChange();
  ...
  if (liveUpdate && isExtensionRuntimeAvailable()) {
    try {
      chrome?.storage[storageEnum].onChanged.addListener(_updateFromStorageOnChanged);
    } catch (error) {
      if (!isExtensionContextInvalidated(error)) {
        console.warn('[storage] failed to subscribe onChanged', key, error);
      }
    }
  }
